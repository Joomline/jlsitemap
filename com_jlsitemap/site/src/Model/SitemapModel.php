...
class SitemapModel extends BaseDatabaseModel
{
	...
	public function generateXSL($type = null)
	{
		...
		if (!isset($this->_xsl[$type]))
		{
			...
			if ($config->get('xsl', 1))
			{
				Factory::getApplication()->getLanguage()->load('com_jlsitemap', JPATH_SITE,
	...
	protected function getUrls()
	{
		if ($this->_urls === null)
		{
			...
			$changefreqValues = ['always'  => 1, 'hourly' => 2, 'daily' => 3, 'weekly' => 4,
			...
			foreach ($rows as $row)
			{
				...
				$link       = ($siteSef && !$item->get('noRoute', false)) ? Route::link('site', $item->get('loc')) : $item->get('loc');
				$level      = count(explode('/', $link)) - 1;
				$key        = (empty($link)) ? '/' : $link;
				$loc        = $siteRoot . $link;
				...
				if (!in_array($key, $filterMenuHomes))
				{
					...
					{
						$exclude[] = ['type' => Text::_('COM_JLSITEMAP_EXCLUDE_UNKNOWN'),
	...
	protected function getMenuItems($multilanguage = false, $menutypes = false, $siteRobots = null, $guestAccess = array())
	{
		if ($this->_menuItems === null)
		{
			...
			$query = $db->getQuery(true)
				->select(['m.id', 'm.menutype', 'm.title', 'm.link', 'm.type', 'm.published', 'm.access', 'm.home', 'm.params', 'm.language',
	...
	public function delete()
	{
		...
		$filename = $this->getConfiguration()->get('filename', 'sitemap');
		$file     = Path::clean(JPATH_ROOT . '/' . $filename . '.xml');
		if (\is_file($file) && !File::delete($file)) return false;
		...
		{
			$path = Path::clean(JPATH_ROOT.'/'.$file);
			if (\is_file($path) && !File::delete($path)) return false;
		}
